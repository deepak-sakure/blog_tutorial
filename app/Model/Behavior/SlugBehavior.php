...
/**
 * File used as slug behavior
 *
 * Contains code needed mainly for slug behavior
 *
...
class SlugBehavior extends ModelBehavior
{
    ...
    function beforeSave(&$model)
    {
        ...
        if (!$model->hasField($this->settings[$model->name]['slug'])) {
            $model->invalidate($this->settings[$model->name]['label'][0], __('The "' . $model->name . '" model does not contain desired field to store slug.', true));
            return false;
        }

        // If user wants to overwrite slug (in case of photo editing) or model's primary field's value is empty (in case of photo adding)
        if ($this->settings[$model->name]['overwrite'] || empty($model->{$model->primaryKey})) {
            ...
            if (empty($label)) {
                $model->invalidate($this->settings[$model->name]['label'][0], __('Unable to create slug using given fields.', true));
                return false;
            }
