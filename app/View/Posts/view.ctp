...
<?php foreach ($comments as $comment): 
      if(AuthComponent::user('id') == $post['Post']['user_id'] || AuthComponent::user('role') == 'admin') {   ?>
          ...
          <div><b><?php echo empty($comment['url'])?$comment['name']:$this->html->link($comment['name'],$comment['url']); ?></b><span style="font-size:11px;padding-left:10px;"><?php echo $comment['created'];//.",".$comment['published']; ?></span>

          <span><?php if($comment['published'] == 0) {
                          echo $this->html->link('Publish', array('controller' => 'comments', 'action' => 'publish', $comment['id'], $post['Post']['id']));
                      }
          echo "&nbsp;&nbsp".$this->html->link('Edit', array('controller' => 'comments', 'action' => 'edit', $comment['id']));
          echo "&nbsp;&nbsp".$this->Form->postLink('Delete', array('controller' => 'comments', 'action' => 'delete', $comment['id'], $post['Post']['id']), array('confirm' => 'Are you sure?'));
          
      ?></span><br/>
          <?php echo $comment['body']; ?><br/>
          </div><br/>

<?php } else if($comment['published']) {  ?>
          
          <div><b><?php echo empty($comment['url'])?$comment['name']:$this->html->link($comment['name'],$comment['url']); ?></b><span style="font-size:11px;padding-left:10px;"><?php echo $comment['created'];//.",".$comment['published']; ?></span><br/>
          <?php echo $comment['body']; ?><br/>
          </div><br/>
